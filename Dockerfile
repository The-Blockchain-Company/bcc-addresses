...
COPY . .
RUN stack install --flag bcc-addresses:release
...
RUN bcc-address --bash-completion-script `which bcc-address` > /etc/bash_completion.d/bcc-address
RUN echo "source /etc/bash_completion.d/bcc-address" >> ~/.bashrc
RUN echo "bcc-address --help" >> ~/.bashrc
ENTRYPOINT ["bcc-address"]
