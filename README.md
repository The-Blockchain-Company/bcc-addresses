<p align="center">
  <big><strong>Bcc Addresses</strong></big>
</p>

<p align="center">
  <a href="https://github.com/The-Blockchain-Company/bcc-addresses/releases"><img src="https://img.shields.io/github/v/release/The-Blockchain-Company/bcc-addresses?color=%239b59b6&label=RELEASE&sort=semver&style=for-the-badge"/></a>
  <a href="https://www.npmjs.com/package/bcc-addresses"><img src="https://img.shields.io/npm/v/bcc-addresses?color=%239b59b6&style=for-the-badge"/></a>
  <br>
  <a href='https://github.com/The-Blockchain-Company/bcc-addresses/actions?query=workflow%3A"Continuous Integration (Linux)"'><img src="https://img.shields.io/github/workflow/status/The-Blockchain-Company/bcc-addresses/Continuous%20Integration%20(Linux)?style=for-the-badge&label=BUILD%20(Linux)" /></a>
  <a href='https://github.com/The-Blockchain-Company/bcc-addresses/actions?query=workflow%3A"Continuous Integration (Windows)"'><img src="https://img.shields.io/github/workflow/status/The-Blockchain-Company/bcc-addresses/Continuous%20Integration%20(Windows)?style=for-the-badge&label=BUILD%20(Windows)" /></a>
  <a href='https://github.com/The-Blockchain-Company/bcc-addresses/actions?query=workflow%3A"TypeScript NPM Package"'><img src="https://img.shields.io/github/workflow/status/The-Blockchain-Company/bcc-addresses/TypeScript%20NPM%20Package?style=for-the-badge&label=BUILD%20(TypeScript)" /></a>
  <br>
  <a href="https://The-Blockchain-Company.github.io/bcc-addresses/coverage/hpc_index.html"><img src="https://The-Blockchain-Company.github.io/bcc-addresses/coverage/badge.svg" /></a>
</p>
...
API documentation is available [here](https://The-Blockchain-Company.github.io/bcc-addresses/haddock).

## Command-Line

`bcc-address` comes with a command-line interface for Linux. See the [release artifacts](https://github.com/The-Blockchain-Company/bcc-addresses/releases) or [continuous integration artifacts](https://github.com/The-Blockchain-Company/bcc-addresses/actions?query=workflow%3A%22Continuous+Integration%22) to get a pre-compiled binary, or [build a Docker image](#docker-image). The command-line is self explanatory by using `--help` on various commands and sub-commands.
...
```console
$ bcc-address recovery-phrase generate --size 15 > phrase.prv
...
```console
$ bcc-address key from-recovery-phrase Sophie < phrase.prv > root.xsk
...
```console
$ bcc-address key child 1852H/1815H/0H/0/0 < root.xsk | bcc-address key public --with-chain-code > addr.xvk
...
```console
$ bcc-address key child 1852H/1815H/0H/2/0 < root.xsk | bcc-address key public --with-chain-code > stake.xvk
...
```console
$ bcc-address address payment --network-tag testnet < addr.xvk > payment.addr
...
```console
$ bcc-address address delegation $(cat stake.xvk) < payment.addr > payment-delegated.addr
...
```console
$ bcc-address address stake --network-tag testnet < stake.xvk > stake.addr
...
``` console
$ bcc-address key from-recovery-phrase Shared < phrase.prv > root_shared.xsk
```

Now generate payment verification key (`role=0` is used). Please note that purpose `1854H` is used for multisig.

```console
$ bcc-address key child 1854H/1815H/0H/0/0 < root_shared.xsk | bcc-address key public --without-chain-code > addr_shared.vk
...
```console
$ bcc-address key child 1854H/1815H/0H/2/0 < root_shared.xsk | bcc-address key public --without-chain-code > stake_shared.vk
...
```console
$ bcc-address script hash "all [$(cat addr_shared.1.vk), $(cat addr_shared.2.vk)]" > script.hash
...
```console
$ bcc-address script hash "any [$(cat addr_shared.1.xvk), $(cat addr_shared.2.xvk)]"
script1gr69m385thgvkrtspk73zmkwk537wxyxuevs2u9cukglvtlkz4k
```
...
```console
$ bcc-address script hash "at_least 1 [$(cat addr_shared.1.xvk), $(cat addr_shared.2.xvk)]"
...
```console
$  bcc-address script hash "all [$(cat addr_shared.1.xvk), $(cat addr_shared.2.xvk), active_from 100, active_until 120]"
```
</details>
...
```console
$  bcc-address script validate "at_least 1 [$(cat addr_shared.1.xvk), $(cat addr_shared.2.xvk), $(cat addr_shared.2.xvk)]"
Validated.

$  bcc-address script validate --recommended  "at_least 1 [$(cat addr_shared.1.xvk), $(cat addr_shared.2.xvk), $(cat addr_shared.2.xvk)]"
...
```console
$ bcc-address script preimage "all [addr_shared_vkh1zxt0uvrza94h3hv4jpv0ttddgnwkvdgeyq8jf9w30mcs6y8w3nq, addr_shared_vkh1y3zl4nqgm96ankt96dsdhc86vd5geny0wr7hu8cpzdfcqskq2cp]"
...
$  bcc-address script preimage "all [addr_shared_vkh1zxt0uvrza94h3hv4jpv0ttddgnwkvdgeyq8jf9w30mcs6y8w3nq, active_from 100, active_until 150]"
...
```console
$ bcc-address address payment --network-tag testnet < script.hash > script.addr
addr_test1wqqggtajwkxjgf58v452jz6jl87lt32w3mhez5hd7xz6hugp80tta
```
</details>

<details>
  <summary>Correspondence between keys in bcc-addresses and bcc-cli (<strong>key.xsk key.xvk key.vk key.hash</strong>)</summary>
...
$ bcc-address key from-recovery-phrase Sophie < recovery-phrase.prv > root.xprv
...
$ bcc-address key child 1852H/1815H/0H/0/0 < root.xprv > key.xsk
...
Create extended signing key using bcc-cli
$ bcc-cli key convert-bcc-address-key --sophie-payment-key --signing-key-file key.xsk --out-file key.skey
{
    "type": "PaymentExtendedSigningKeySophie_ed25519_bip32",
    "description": "",
...
Create corresponding verification key using bcc-cli
$ bcc-cli key verification-key --signing-key-file key.skey --verification-key-file key.vkey
{
    "type": "PaymentExtendedVerificationKeySophie_ed25519_bip32",
    "description": "",
...
Create verification key hash using bcc-cli
$ bcc-cli address key-hash --payment-verification-key-file key.vkey > key.hash
0185545935760c5e370d01e6f4fedbb89b7fd79e115f2837cfab9ea8

Alternatively, we can create non-extended key
$ bcc-address key public --without-chain-code < key.xsk > key.vk
...
$ bcc-cli address key-hash --payment-verification-key $(cat key.vk) > key1.hash
0185545935760c5e370d01e6f4fedbb89b7fd79e115f2837cfab9ea8

Within bcc-addresses one can get cborhex of verification key (with chain code)
$ bcc-address key public --with-chain-code < key.xsk | bech32
...
Within bcc-addresses one can get cborhex of verification key (without chain code)
$ bcc-address key public --without-chain-code < key.xsk | bech32
...
$ bcc-address key public --without-chain-code < key.xsk | bcc-address key hash | bech32
0185545935760c5e370d01e6f4fedbb89b7fd79e115f2837cfab9ea8
...
```console
$ docker run --rm bcc-address recovery-phrase generate --size 15
...
```console
$ echo "addr1gqtnpvdhqrtpd4g424fcaq7k0ufuzyadt7djygf8qdyzevuph3wczvf2dwyx5u" | docker run --rm -i bcc-addresses address inspect
{
...
[coding standards](https://github.com/The-Blockchain-Company/adrestia/wiki/Coding-Standards).

<hr />

<p align="center">
  <a href="https://github.com/The-Blockchain-Company/bcc-addresses/blob/master/LICENSE"><img src="https://img.shields.io/github/license/The-Blockchain-Company/bcc-addresses.svg?style=for-the-badge" /></a>
</p>
