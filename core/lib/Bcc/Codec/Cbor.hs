...
-- License: Apache-2.0
--
-- These are (partial) CBOR decoders for Cole binary types. Note that we
...
{-------------------------------------------------------------------------------
                       Cole Address Binary Format

In the composition of a Bcc address, the following functions concern the
"Derivation Path" box.
...
-- | Encode a public key to a corresponding Bcc Address. The encoding of the
...
encodeAddressPayload payload = mempty
    ...
    <> CBOR.encodeTag 24 -- Hard-Coded Tag value in bcc-sl
...
decodeAddress = do
    ...
    tag <- CBOR.decodeTag
        -- Mysterious hard-coded tag bcc-sl seems to so much like
...
-- | Cole CBOR encodings often have CBOR nested in CBOR. This helps decoding
