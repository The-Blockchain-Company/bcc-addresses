...
spec = do
    ...
    describe "Golden Tests for Cole Addresses w/ random scheme (Mainnet)" $ do
    ...
    describe "Golden Tests for Cole Addresses w/ random scheme (Testnet)" $ do
...
decodeDerivationPathTest DecodeDerivationPath{..} =
  ...
  where
    ...
    key = genMasterKeyFromMnemonic mw () :: Cole 'RootK XPrv
