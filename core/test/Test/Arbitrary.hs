...
import Bcc.Address.Style.Cole
    ( Cole, coleMainnet, coleStaging, coleTestnet )
...
instance Arbitrary (Cole 'PaymentK XPub) where
    shrink _ = []
    arbitrary = do
...
instance Arbitrary (Sophie 'PaymentK XPub) where
    shrink _ = []
    arbitrary = do
        mw <- SomeMnemonic <$> genMnemonic @15
        bytes <- BA.convert . BS.pack <$> (choose (0, 32) >>= vector)
...
instance Arbitrary (Sophie 'DelegationK XPub) where
    shrink _ = []
    arbitrary = do
        mw <- SomeMnemonic <$> genMnemonic @15
        bytes <- BA.convert . BS.pack <$> (choose (0, 32) >>= vector)
        let rootK = genMasterKeyFromMnemonic mw bytes
        acctK <- deriveAccountPrivateKey rootK <$> arbitrary
        let delegationK = Sophie.deriveDelegationPrivateKey acctK
...
instance Arbitrary Sophie.Role where
